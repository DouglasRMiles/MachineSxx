...
#ifdef DEBUGGING
//#define IFDEBUG(mask,CPP) if ((mask & debugflags)!=0) CPP 
#define IFDEBUG(mask,CPP) CPP 
#else
#define IFDEBUG(mask,CPP) 
#endif
...
int Goal::solve(Prolog* m, Prog1Pred* p, bool interactive, int results, int level, TermVarMapping* map) {
	...
	bool cutted = false;
	bool success = false;
	IFDEBUG(SOLVE,{ indent(level); str << "solve@" << level << ": ";
			 this->print(str); str << "\n";});
	...
	for(Prog1Pred* q = p; q != NULL; q = q->pcdr) {
		...
		int nextLevel = 1;
		m->Undo(t);
		IFDEBUG(SOLVE,{ indent(level); str << "  try:"; c->print(str); str << "\n";});
		if(head->unifyTerm(m, c->head)) {
			...
			if(gdash == NULL) {
				...
				if(waz == 0) {
					IFDEBUG(SOLVE,{ indent(level); str << "  parent fails.\n";});
				}
			}
		} else {
			IFDEBUG(SOLVE,{ indent(level); str << "  nomatch.\n";});
			if(interactive) {
				if(results!=0) {
